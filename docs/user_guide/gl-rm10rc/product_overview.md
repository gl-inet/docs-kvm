...
Comet 5G features Wi-Fi Near-Field management that allows quick local configuration and management without the need for wired connections. By toggling the dedicated Wi-Fi switch, the device activates its built-in AP and generates a local Service Set Identifier (SSID). Users can connect to this network to securely access the management interface for setup or maintenance.
