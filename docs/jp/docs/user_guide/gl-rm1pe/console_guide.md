...
ここで、ホスト名やIPアドレスなど、Comet PoEのネットワークの詳細を確認できます。
...
- **Ethernet**: Comet PoE がイーサネット ケーブル経由で上流のネットワーク デバイスに接続すると、そのイーサネット IP アドレスがここに表示されます。 IP アドレスまたは右矢印をクリックすると、イーサネットの詳細が表示されます。
...
Comet PoE の端末にアクセスして詳細設定を行うことができます。 「**Access**」をクリックします。
...
Comet PoE がクラウドにバインドされると、コンソールには次のようにクラウドのステータスが表示されます。
