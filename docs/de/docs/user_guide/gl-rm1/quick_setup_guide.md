# Comet (GL-RM1) V1/V2 Schnellstart

Sehen Sie sich dieses Video an oder folgen Sie den Schritten unten, um Ihren Comet einzurichten.
...
1. Verbinden Sie den Comet mit der Stromquelle.

    ![power on](https://static.gl-inet.com/docs/kvm/user_guide/gl-rm1/quick_setup/02_power-on.jpg){class="glboxshadow"}

2. Verbinden Sie den HD IN-Anschluss des Comet mit dem HD OUT-Anschluss von Geraet B ueber ein HDMI-Kabel.

    ![Connect the HD cable](https://static.gl-inet.com/docs/kvm/user_guide/gl-rm1/quick_setup/03_hdmi.jpg){class="glboxshadow"}

3. Verbinden Sie den USB-Device-Anschluss des Comet ueber ein USB-Kabel mit dem USB-Anschluss von Geraet B.

    ![Connect the USB port](https://static.gl-inet.com/docs/kvm/user_guide/gl-rm1/quick_setup/04_usb-cable.jpg){class="glboxshadow"}

4. Verbinden Sie den Ethernet-Anschluss des Comet mit einer Netzwerkquelle.

    ![Connect to network](https://static.gl-inet.com/docs/kvm/user_guide/gl-rm1/quick_setup/05_ethernet.jpg){class="glboxshadow"}

5. Die Geraeteverbindung ist abgeschlossen. Jetzt koennen Sie lokal oder aus der Ferne auf die Konsole des Comet zugreifen.

## Lokaler Zugriff

Es gibt zwei Moeglichkeiten, im lokalen Netzwerk auf Comet zuzugreifen: ueber den Domainnamen oder ueber die IP-Adresse.

Stellen Sie vor dem Zugriff sicher, dass sich Ihr steuerndes Geraet im selben LAN wie Comet befindet.
...
3. Danach koennen Sie lokal auf die Konsole des Comet und auf das gesteuerte Geraet zugreifen.
...
1. Suchen Sie die IP-Adresse des Comet im uebergeordneten Router.

    Beispiel: Comet ist ueber ein Ethernet-Kabel mit dem LAN-Port eines GL.iNet Routers GL-AXT1800 verbunden.

    Melden Sie sich im Web-Admin-Panel des GL-AXT1800 an und suchen Sie die IP-Adresse des Comet in der Client-Liste, wie unten gezeigt.
...
2. Oeffnen Sie im Browser einen neuen Tab und geben Sie die IP-Adresse des Comet in die Adressleiste ein. Sie werden zur GLKVM-Anmeldeseite weitergeleitet. Geben Sie das Admin-Passwort ein.
...
Es gibt mehrere Moeglichkeiten, aus der Ferne auf Comet zuzugreifen: ueber den Cloud-Dienst, die GLKVM App, Tailscale, ZeroTier und NetBird.
...
2. Fernzugriff ueber den Cloud-Dienst.
    ...
    Danach koennen Sie ueber die Cloud aus der Ferne auf das KVM und das gesteuerte Geraet zugreifen, ohne die App zu installieren.
...
3. Binden Sie Ihr Geraet.
    ...
    ??? "Auto Discover"
        ...
        Dies muss im lokalen Netzwerk erfolgen. Stellen Sie sicher, dass sich Ihr steuerndes Geraet im selben LAN wie Comet befindet.
...
Comet ist in Tailscale integriert und ermoeglicht Fernzugriff ueber ein virtuelles Tailscale-Netzwerk.

Navigieren Sie in der Konsole des Comet zu **Apps Center** -> **Tailscale**, aktivieren Sie Tailscale und binden Sie Ihren Comet an Ihr Tailscale-Konto.

Binden Sie anschliessend Ihr steuerndes Geraet an dasselbe Konto. Danach koennen Sie aus der Ferne auf Ihren Comet zugreifen, indem Sie dessen **Tailscale virtual IP** in einem Webbrowser auf dem steuernden Geraet eingeben, ohne die GLKVM App zu installieren.

Details finden Sie [hier](../../faq/remote_access_via_tailscale.md){target="_blank"}.

### ZeroTier

Comet ist in ZeroTier integriert und ermoeglicht Fernzugriff ueber ein virtuelles ZeroTier-Netzwerk.

Navigieren Sie in der Konsole des Comet zu **Apps Center** -> **ZeroTier** und aktivieren Sie ZeroTier.

Treten Sie anschliessend sowohl mit Comet als auch mit Ihrem steuernden Geraet demselben ZeroTier-Netzwerk bei (mithilfe einer 16-stelligen alphanumerischen Network ID). Danach koennen Sie aus der Ferne auf Ihren Comet zugreifen, indem Sie dessen **ZeroTier IP** in einem Webbrowser auf dem steuernden Geraet eingeben, ohne die GLKVM App zu installieren.

Details finden Sie [hier](../../faq/remote_access_via_zerotier.md){target="_blank"}.

### NetBird

Comet ist in NetBird integriert und ermoeglicht Fernzugriff ueber ein virtuelles NetBird-Netzwerk.

Navigieren Sie in der Konsole des Comet zu **Apps Center** -> **NetBird**, aktivieren Sie NetBird und binden Sie Ihren Comet an Ihr NetBird-Konto.

Binden Sie anschliessend Ihr steuerndes Geraet an dasselbe Konto. Danach koennen Sie aus der Ferne auf Ihren Comet zugreifen, indem Sie dessen **NetBird virtual IP** in einem Webbrowser auf dem steuernden Geraet eingeben, ohne die GLKVM App zu installieren.
